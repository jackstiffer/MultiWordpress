...
- [ ] **Phase 2: CLI Core + First Site E2E** — Full CLI surface (`wp-create`/`wp-delete`/`wp-list`/`wp-stats`/`wp-logs`/`wp-exec`/`wp-backup`/`wp-restore`) provisions one real domain end-to-end and validates the Cloudflare + Super Page Cache promise.
- [ ] **Phase 3: Operational Tooling** — Cron stagger, backup smoke-test, and the rest of the daily-driver verbs needed to run 5+ sites without manual babysitting.
...
**Goal**: A complete CLI provisions, lists, backs up, and tears down sites; the first real domain is live through the CLI and the Cloudflare + Super Page Cache strategy delivers near-static-file TTFB for logged-out reads.
**Depends on**: Phase 1
**Requirements**: CLI-01, CLI-02, CLI-03, CLI-04, CLI-05, CLI-06, CLI-07, CLI-08, CLI-09, CLI-10, CLI-11, CLI-12, CLI-13, STATE-01, STATE-02, STATE-03, STATE-04, PERF-01, PERF-02, HARD-02
**Success Criteria** (what must be TRUE):
  ...
  3. `wp-list` shows all sites with slug/domain/status/port/redis DB; `wp-list --secrets <slug>` re-displays creds without leaking to shell history; `wp-stats` prints host CPU/mem/disk plus per-container stats for every `wp-*`; `wp-logs <site> [--follow]` and `wp-exec <site> <wp-cli-args>` work; `wp-delete [--archive]` removes container + DB + user + secrets and prints exact Caddy/Cloudflare cleanup snippets.
  4. `wp-backup <site>` writes a per-site DB dump (`mysqldump --single-transaction --quick`, never global lock) plus `wp-content` tarball to `/opt/wp/backups/<slug>/<timestamp>/`, and `wp-restore <site> <backup-path>` round-trips successfully on the live first site.
...
**Goal**: Adding the 5th–10th site is painless because cron is staggered, backups are proven by automated smoke-test, and resource usage stays inside the 4 GB / 1 vCPU envelope.
**Depends on**: Phase 2
...
**Success Criteria** (what must be TRUE):
  ...
  2. A weekly cron-driven backup-restore smoke test backs up a real site, restores it into a throwaway slug, asserts the homepage returns 200, and tears down — failures are surfaced in a log the operator checks.
  3. With 5 real sites running and Cloudflare absorbing logged-out reads, `docker stats` shows the WP cluster sustained under 2 GB resident and under 50% of one vCPU; AudioStoryV2 has not been OOM-killed or restarted.
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  ...
  3. README walks a new operator from zero to a live site (prerequisites, Caddy + Cloudflare assumptions, full lifecycle, backup/restore drill); a Caddy snippet template + Cloudflare DNS row guide is included; a scaling-cliff doc names the four warning signs that this single-VM design has been outgrown.
**Plans**: TBD
**UI hint**: yes
...
| 2. CLI Core + First Site E2E | 19 | CLI-01..13, STATE-01..04, PERF-01, PERF-02, HARD-02 |
...
- **Phase 3 is small by REQ count (1)** but is where the multi-site invariants get *proven* under load (cron stagger, backup smoke-test, budget validation with 5 sites). Most of its content lives in success criteria, not new requirement IDs — by design, since the v1 list under-specifies operational hardening.
